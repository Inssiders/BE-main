...
public interface PostRepository extends JpaRepository<Post, Long> {
  ...
  @Query(
      "SELECT new com.example.webtemplate.post.dto.PostGetIdResponseDTO(p.id, p.createdAt) "
          + "FROM Post p WHERE p.createdAt >= :since")
  List<PostGetIdResponseDTO> findPostsByCreatedAtAfter(@Param("since") LocalDateTime since);

  @Query(
      "SELECT new com.example.webtemplate.post.dto.PostGetIdResponseDTO(p.id, p.createdAt) FROM Post p")
