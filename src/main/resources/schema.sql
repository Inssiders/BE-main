...
drop table if exists email_auth_codes cascade;
...
create table accounts (is_deleted boolean not null, created_at timestamp(6) not null, deleted_at timestamp(6), id bigint not null, updated_at timestamp(6), account_type varchar(255) check (account_type in ('PASSWORD','OTHER')), email varchar(255), password varchar(255), provider_user_id varchar(255), role varchar(255) check (role in ('USER','CONTENT_MANAGER','SUPERADMIN')), primary key (id));
...
create table email_auth_codes (created_at timestamp(6) not null, expired_at timestamp(6), code varchar(255) not null, email varchar(255) not null, primary key (email));
...
create table refresh_tokens (account_id bigint not null, token varchar(255), primary key (account_id));
create table tags (created_at timestamp(6), id bigint not null, name varchar(255) not null, primary key (id));
create table user_profiles (account_visible boolean not null, follower_visible boolean not null, account_id bigint not null unique, created_at timestamp(6) not null, id bigint not null, updated_at timestamp(6), bio varchar(255), nickname varchar(255), profile_url varchar(255), primary key (id));
